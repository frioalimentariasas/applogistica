...
export async function uploadInventoryCsv(formData: FormData): Promise<{ success: boolean; message: string; processedCount: number; errors: string[] }> {
    try {
        ...
        // Process files sequentially to avoid overwhelming server resources with parallel processing.
        for (const file of files) {
            if (file.size === 0) {
                allResults.push({ success: false, fileName: file.name, error: "Archivo vacío." });
            ...
            try {
                ...
                allResults.push({ success: true, fileName: file.name, error: null });

            } catch (error) {
                const errorMessage = error instanceof Error ? error.message : String(error);
                console.error(`Error al procesar el archivo ${file.name}:`, errorMessage);
                allResults.push({ success: false, fileName: file.name, error: errorMessage });
            }
        }
        
        const processedCount = allResults.filter(r => r.success === true).length;
...
export async function getInventoryReport(
    ...
    try {
        ...
        if (error instanceof Error) {
            if (error.message.includes('needs an index')) {
